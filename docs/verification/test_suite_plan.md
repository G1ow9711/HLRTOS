...
| `tests/coupling/` | 跨模块耦合测试 | 矩阵 `C-001` 到 `C-044` |
...
- 真实硬件验收：先按目标板修改 `docs/verification/hardware_smoke/hardware_smoke_preflight.json` 并运行 `tools/verify/check_hardware_smoke_preflight.py --target STM32|DSP`；若要一次检查两类目标则使用默认 `all`。再用 `tools/verify/check_hardware_smoke_raw_log_schema.py` 检查 `raw_log_schema.md`、生成器字段常量、schema C 头文件、完整报告 emitter、raw-log 直检脚本和采集指南一致；随后用 `tools/verify/run_hardware_smoke_capture.py --target STM32|DSP` 预演采集流水线；确认命令会连接真实板卡并产出 raw log 后追加 `--execute`，执行器会在采集后先运行 raw log 直检，再生成最终证据。手动采集时，按 `docs/verification/hardware_smoke/raw_log_schema.md` 和 `docs/verification/hardware_smoke/collection_checklist.md` 整理原始日志，先用 `tools/verify/check_hardware_smoke_raw_log.py --target STM32|DSP --input <raw-log>` 检查 raw log，再用 `tools/verify/generate_hardware_smoke_evidence.py` 生成含 raw-log SHA-256 的 `stm32_board_smoke.md` 和 `dsp_board_smoke.md`，最后运行 `tools/verify/check_hardware_smoke_evidence.py`。
...
| 真实板级证据 | `docs/verification/hardware_smoke/`、`examples/hardware_smoke/mrt_hardware_smoke_log_schema.h`、`examples/hardware_smoke/mrt_hardware_smoke_log.h`、`examples/hardware_smoke/mrt_hardware_smoke_report.h`、`tools/verify/check_hardware_smoke_preflight.py`、`tools/verify/check_hardware_smoke_raw_log_schema.py`、`tools/verify/check_hardware_smoke_raw_log.py`、`tools/verify/run_hardware_smoke_capture.py`、`tools/verify/generate_hardware_smoke_evidence.py`、`tools/verify/check_hardware_smoke_evidence.py`、`tests/unit/test_hardware_smoke_log.c`、`tests/unit/test_hardware_smoke_report.c`、`tests/static/test_hardware_smoke_capture_runner.py`、`tests/static/test_hardware_smoke_raw_log_schema.py`、`tests/static/test_hardware_smoke_raw_log_checker.py`、`tests/static/test_hardware_smoke_evidence_checker.py`、`tests/static/test_hardware_smoke_evidence_generator.py` | `R-003`、`R-004`、`R-011`、`C-038`、`C-040` 到 `C-044`；包含 dry-run/execute 采集顺序、目标级预检过滤、raw-log 字段 schema 对齐、raw log 直检、schema checker 对完整报告 emitter 与 raw-log 直检脚本的字段覆盖、板级输出 helper、raw-log SHA-256 派生与错配拒绝 |
