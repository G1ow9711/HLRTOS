...
`run_release_verification.py` 作为最终验收入口，默认执行 host、静态、原型、STM32 汇编骨架、DSP 汇编骨架、DSP C2000 工程骨架、embedded smoke、硬件 smoke 预检配置、硬件采集执行器自测、硬件证据校验脚本自测和原始日志生成器自测；`--require-hardware` 会把真实 STM32/DSP 板级证据 gate 纳入同一条链路。
