...
| R-002 | 复现类似嵌入式 RTOS | 提供任务、调度、同步、通信、定时、内存、低功耗、trace、移植层 | 设计草案第 3、4 节 | Foundation + scheduler + queues + semaphores + mutexes + event groups + task notifications + software timers + stream/message buffers 已实现；新增通信源码见 `include/myrtos/mrt_stream_buffer.h`、`src/kernel/mrt_stream_buffer.c`、`include/myrtos/mrt_message_buffer.h`、`src/kernel/mrt_message_buffer.c` | `python tools\run_host_tests.py`：44 个 host 测试目标通过 | 手册章节 3-18 待写；STM32/DSP 详细移植步骤列入 R-011 | 部分实现 |
...
| R-008 | 所有功能必须测试 | 每个 public API 和核心内部算法有自动化测试 | 设计草案第 7 节 | Foundation、scheduler、queue、semaphore、mutex、event group、task notification、software timer、stream buffer、message buffer public API 已有 host/sim/coupling 测试 | `python tools\run_host_tests.py`：44 个 host 测试目标通过；stream/message 新增覆盖 `test_stream_buffer_create_static`、`test_stream_buffer_send_receive`、`test_stream_buffer_isr`、`test_stream_buffer_isr_wakes_reader`、`test_message_buffer_create_static`、`test_message_buffer_send_receive`、`test_message_buffer_isr` | 测试报告章节待写 | 部分验证 |
| R-009 | 所有耦合情况测试清楚 | 用耦合矩阵覆盖模块交互、ISR、超时、内存失败、trace、tickless | 设计草案第 7.3 节；耦合矩阵文件 | 已验证 scheduler + tick + ready/delay list、queue + task/ISR、semaphore + task/ISR、mutex + priority inheritance/ownership、event group + multi-waiter/clear/ISR、task notification + overwrite/wait/ISR、software timer + kernel tick/pending FIFO、stream buffer + wrap/ISR reader wake、message buffer + packet capacity/ISR reader wake；内存、trace、tickless、任务删除、真实端口仍待后续模块 | `test_stream_buffer_send_receive`、`test_stream_buffer_isr_wakes_reader`、`test_message_buffer_send_receive`、`test_message_buffer_isr` 已加入耦合证据；`python tools\run_host_tests.py`：44 个 host 测试目标通过 | 测试报告章节待写 | 部分验证 |
| R-010 | C 版大而全范围 | 包含 stream/message buffer、tickless、trace、MPU 预留、多 heap 策略等高级模块 | 设计草案第 4 节 | stream buffer 和 message buffer 已实现静态创建、任务态收发、ISR 收发、读者唤醒和容量边界；tickless、trace、MPU 预留、多 heap 策略仍待后续计划 | `test_stream_buffer_create_static`、`test_stream_buffer_send_receive`、`test_stream_buffer_isr`、`test_stream_buffer_isr_wakes_reader`、`test_message_buffer_create_static`、`test_message_buffer_send_receive`、`test_message_buffer_isr` 通过 | 高级模块手册待写 | 部分实现 |
