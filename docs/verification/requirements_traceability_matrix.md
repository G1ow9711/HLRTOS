...
| R-002 | 复现类似嵌入式 RTOS | 提供任务、调度、同步、通信、定时、内存、低功耗、trace、移植层 | 设计草案第 3、4 节 | Foundation + scheduler + task lifecycle APIs + queues + semaphores + mutexes + event groups + task notifications + software timers + stream/message buffers + heap + fixed memory pool + dynamic queue/task/sync/event/timer/buffer allocation + tickless idle + trace hook + assert hook 已实现；STM32/DSP 端口契约 helper 已实现；新增端口源码见 `include/myrtos/portable/mrt_port_stm32_cm.h`、`src/portable/stm32_cm/mrt_port_stm32_cm.c`、`include/myrtos/portable/mrt_port_dsp_c28x.h`、`src/portable/dsp_c28x/mrt_port_dsp_c28x.c` | `python tools\run_host_tests.py`：64 个 host 测试目标通过 | `docs/manual/MyRTOS_Reference_Manual_zh.md` 覆盖任务 API、同步通信 API、内存、tickless/trace/assert、STM32/DSP 详细移植步骤和动态对象生命周期 | 部分实现 |
...
| R-008 | 所有功能必须测试 | 每个 public API 和核心内部算法有自动化测试 | 设计草案第 7 节 | Foundation、scheduler、task lifecycle、queue、semaphore、mutex、event group、task notification、software timer、stream buffer、message buffer、heap、memory pool、dynamic queue/task/sync/event/timer/buffer allocation、tickless、trace、assert、STM32 port helper、DSP port helper public API 已有 host/sim/coupling 测试 | `python tools\run_host_tests.py`：64 个 host 测试目标通过；本阶段新增 `test_sync_dynamic_allocation`、`test_event_timer_dynamic_allocation`、`test_buffer_dynamic_allocation` | `docs/verification/final_verification_report.md` 记录最新 host/static 结果 | 部分验证 |
| R-009 | 所有耦合情况测试清楚 | 用耦合矩阵覆盖模块交互、ISR、超时、内存失败、trace、tickless | 设计草案第 7.3 节；耦合矩阵文件 | 已验证 scheduler + task lifecycle + tick + ready/delay list、task + heap dynamic allocation、task ResumeFromISR、queue + task/ISR、queue + heap dynamic allocation、dynamic semaphore/mutex/event/timer/buffer heap failure and delete semantics、semaphore + task/ISR、mutex + priority inheritance/ownership、event group + multi-waiter/clear/ISR、task notification + overwrite/wait/ISR、software timer + kernel tick/pending FIFO、software timer + tickless compensation、stream buffer + wrap/ISR reader wake、message buffer + packet capacity/ISR reader wake、heap allocation failure、heap coalescing、trace + task switch、trace + queue、assert hook、STM32 stack/tick/BASEPRI helper、DSP stack/context helper；真实板级端口仍待后续模块 | `test_sync_dynamic_allocation`、`test_event_timer_dynamic_allocation`、`test_buffer_dynamic_allocation` 已加入动态对象耦合证据；`python tools\run_host_tests.py`：64 个 host 测试目标通过 | `docs/verification/coupling_test_matrix.md` 更新 C-017/C-020/C-022/C-023/C-032 | 部分验证 |
| R-010 | C 版大而全范围 | 包含 stream/message buffer、tickless、trace、MPU 预留、多 heap 策略等高级模块 | 设计草案第 4 节 | stream/message buffer 与动态创建路径已实现；多 heap 策略已实现线性、free-list、coalescing 三种模式；固定块 memory pool、动态队列、动态任务和动态同步/定时对象堆分配已实现；tickless idle、trace hook、assert hook 已实现；MPU 预留仍待后续计划 | `test_stream_buffer_create_static`、`test_stream_buffer_send_receive`、`test_stream_buffer_isr`、`test_stream_buffer_isr_wakes_reader`、`test_message_buffer_create_static`、`test_message_buffer_send_receive`、`test_message_buffer_isr`、`test_buffer_dynamic_allocation`、`test_heap_linear`、`test_heap_free_list`、`test_heap_coalescing`、`test_memory_pool`、`test_queue_dynamic_allocation`、`test_tickless_expected_idle`、`test_tickless_timer_compensation`、`test_trace_task_switch`、`test_trace_queue`、`test_assert_hook` 通过 | `docs/manual/MyRTOS_Reference_Manual_zh.md` 覆盖高级模块 API 和动态对象限制 | 部分实现 |
