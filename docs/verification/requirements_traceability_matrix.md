...
| R-002 | 复现类似嵌入式 RTOS | 提供任务、调度、同步、通信、定时、内存、低功耗、trace、移植层 | 设计草案第 3、4 节 | Foundation + scheduler + task lifecycle APIs + queues + semaphores + mutexes + event groups + task notifications + software timers + stream/message buffers + heap + fixed memory pool + dynamic queue/task/sync/event/timer/buffer allocation + tickless idle + trace hook + runtime stats + assert hook 已实现；互斥锁 timeout 优先级回滚已实现；STM32/DSP 端口契约 helper 已实现；新增端口源码见 `include/myrtos/portable/mrt_port_stm32_cm.h`、`src/portable/stm32_cm/mrt_port_stm32_cm.c`、`include/myrtos/portable/mrt_port_dsp_c28x.h`、`src/portable/dsp_c28x/mrt_port_dsp_c28x.c` | `python tools\run_host_tests.py`：66 个 host 测试目标通过 | `docs/manual/MyRTOS_Reference_Manual_zh.md` 覆盖任务 API、同步通信 API、内存、tickless/trace/runtime stats/assert、STM32/DSP 详细移植步骤和动态对象生命周期 | 部分实现 |
