...
| C-032 | 手册 + API | 每个 public API | 手册有原型、参数、返回值、示例、上下文限制 | 文档检查 | 已验证：`python tools\verify\check_api_manual_coverage.py` 覆盖 `docs/api/myrtos_api_catalog.md` 中 126 个 API 条目，并检查每个条目包含函数原型、功能说明、参数、返回值、调用上下文、阻塞行为、ISR 限制、配置宏、调用示例、常见错误；同脚本验证 STM32/DSP 移植章节包含工具链、启动文件、向量表、tick、上下文切换、栈布局、临界区、低功耗、示例、排错；本阶段手册补充 timer service 命令队列语义、动态对象精确原型、静态删除拒绝、等待者/持锁忙删除、活动定时器删除先停止、动态缓冲单堆块布局和当前无缓冲删除 API 的限制 |
