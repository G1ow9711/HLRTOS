...
- Embedded smoke：STM32 smoke 可由 ARM GCC 交叉编译，其中包含 `mrt_port_stm32_cm_context.S` 的 SVC/PendSV 汇编入口骨架；DSP smoke/model 可在 host 上编译并运行。
...
- 当前 STM32 证据是交叉编译/link scaffold，含 SVC/PendSV 汇编骨架静态检查和交叉编译证据；DSP 证据是 host-verifiable C28x-style model。
...
| `python tests\static\test_stm32_context_scaffold.py` | STM32 Cortex-M SVC/PendSV 汇编骨架、C 钩子和 smoke 构建接线静态检查通过 |
...
| R-003 | 部分实现 | STM32 栈帧、SysTick reload、BASEPRI helper、MPU 区域规整 helper 已测试；`examples/stm32` 已用 ARM GCC 交叉编译；SVC/PendSV 汇编入口骨架已由 `test_stm32_context_scaffold.py` 静态检查并纳入 embedded smoke 构建 | 真实 SysTick/PendSV/SVC 运行证据和板级 smoke |
...
| R-009 | 部分验证 | C-001 到 C-037 中大量耦合项已有自动化证据，新增 STM32 cross-build smoke、STM32 SVC/PendSV 汇编骨架静态检查、STM32 MPU helper 与 DSP host model smoke 证据；动态 buffer delete 生命周期和写者等待 busy 删除保护已落表 | 真实端口项仍待硬件补证 |
...
- `C-028`、`C-029`、`C-035`：STM32 helper、STM32 SVC/PendSV 汇编骨架静态检查、STM32 MPU helper、STM32 smoke 交叉编译证据。
...
- `C-028`、`C-029`：STM32 helper 已验证，`examples/stm32` 已通过 ARM GCC 交叉编译，`src/portable/stm32_cm/mrt_port_stm32_cm_context.S` 已验证符号和构建接线；真实 SysTick/PendSV/SVC 与 BASEPRI/PRIMASK 硬件行为待板级 smoke。
