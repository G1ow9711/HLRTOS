...
2. 在真实 STM32 板卡运行 `examples/stm32` 派生工程，按 `docs/verification/hardware_smoke/collection_checklist.md` 记录 LED、UART ISR 队列、软件定时器、tickless idle、PendSV/SVC 汇编证据，并填写 `docs/verification/hardware_smoke/stm32_board_smoke.md`。
3. 根据用户指定 DSP 型号补真实 ABI 汇编端口和 timer/software interrupt smoke，按 `docs/verification/hardware_smoke/collection_checklist.md` 采集原始日志，并填写 `docs/verification/hardware_smoke/dsp_board_smoke.md`。
