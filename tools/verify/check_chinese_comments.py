#!/usr/bin/env python3
"""检查 include/src 中 C 代码的中文函数注释覆盖。"""
...
SOURCE_DIRS = [ROOT / "include", ROOT / "src"]
...
def candidate_function_name(signature: str) -> str | None:
    ...
    if name in CONTROL_WORDS:
        return None
    if not (name.startswith("MRT_") or name.startswith("mrt_")):
        return None
    return name
...
def main() -> int:
    ...
    for directory in SOURCE_DIRS:
        for path in sorted(directory.rglob("*")):
            if path.suffix not in {".c", ".h"}:
                continue
            if path.suffix == ".c":
                failures.extend(check_source_file(path))
            else:
    ...
    print("[chinese-comments] include/src function comments covered")
    return 0
